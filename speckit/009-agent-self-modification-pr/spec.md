...
| 009-FR-004 | Merge requires explicit operator approval OR a pre-authorized rule (e.g., "auto-merge if benchmark delta > 0 and changes only `prompts/`"). |
...
| 009-FR-010 | Self-modifications MUST be rate-limited per agent (default: max 1 per day, configurable per team). |
